...
pub async fn select_pages(Extension(repo): RepoExt, 
    ...
    let users_res = service::user::select_pages(repo.clone(), &user_query_param).await;
    users_res.to_result()
}
...
pub async fn insert(Extension(repo): RepoExt, Json(user): Json(User)) -> R<()> {
    let res = service::user::insert(repo.clone(), user).await;
    res.to_result()
}

pub async fn update(Extension(repo): RepoExt, Json(user): Json(User)) -> R<()> {
    let res = service::user::update(repo.clone(), user).await;
    res.to_result()
}
